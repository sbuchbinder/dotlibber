...
def default_file_namer(lib, corner):
    return os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), "output",
                        default_library_namer(lib, corner) + ".lib")
...
class Corner:
    ...
    def __init__(self, attr, characterizer):
        ...
        try:
            ...
            self.constraint_template = LUTTemplate(
                "constraint_template_%dx%d" % (len(x["related_pin_transition"]), len(x["constrained_pin_transition"])),
                "related_pin_transition",
                x["related_pin_transition"],
                "constrained_pin_transition",
                x["constrained_pin_transition"])
            x = self.attr["delay_template"]
            self.delay_template = LUTTemplate(
                "delay_template_%dx%d" % (len(x["input_net_transition"]), len(x["total_output_net_capacitance"])),
...
class Pin:
    ...
    def __init__(self, cell, attr, defaults):
        ...
        if not self.is_analog:
            ...
            related_power_pin = ("related_power_pin", require_values(self, "related_power_pin",
                                                                     map(lambda x: x.name, self.cell.power_pins()),
                                                                     defaults))
            # Assert that we must have a ground pin with the name in our PG pin list
            related_ground_pin = ("related_ground_pin", require_values(self, "related_ground_pin",
                                                                       map(lambda x: x.name, self.cell.ground_pins()),
                                                                       defaults))
            if self.is_bus:
    ...
    def generate_arcs(self, corner):
        if self.related_clock_name not in self.cell.clocks.keys():
            sys.stderr.write(
                "Related clock pin \"%s\" of pin \"%s\" of cell \"%s\" is not defined as a clock. Please give it the \"clock : true\" attribute. Aborting.\n" % (
            ...
            if self.direction == "input":
                ...
                raise Exception(
                    "Should not get here, fix me. You have an inout sequential pin, or something else went wrong.")
    ...
    def emit(self, corner):
        ...
        if self.is_bus:
            ...
            # Only specify the pin if it has custom attributes. Otherwise lib compilation fails
            if bus_attributes:
                output += "\n"
                output += indent("pin ( %s[%d:%d] ) {\n" % (self.name, self.bus_max, self.bus_min)
                                 + indent(bus_attributes) + "}\n")
            output += "}\n"
        else:
            output = "pin (%s) {\n" % self.name + indent(attributes) + "}\n"
        return output
...
class SetupArc:
    ...
    def __init__(self, pin, related_pin, corner):
        ...
        self.rise_constraint = generate_data_table("rise_constraint", "setup_rising", pin, related_pin,
                                                   corner.constraint_template, corner)
        self.fall_constraint = generate_data_table("fall_constraint", "setup_rising", pin, related_pin,
                                                   corner.constraint_template, corner)
...
class HoldArc:
    ...
    def __init__(self, pin, related_pin, corner):
        self.pin = pin
        self.related_pin = related_pin
        self.rise_constraint = generate_data_table("rise_constraint", "hold_rising", pin, related_pin,
                                                   corner.constraint_template, corner)
        self.fall_constraint = generate_data_table("fall_constraint", "hold_rising", pin, related_pin,
                                                   corner.constraint_template, corner)
...
class ClockToQArc:
    ...
    def __init__(self, pin, related_pin, corner):
        self.pin = pin
        self.related_pin = related_pin
        self.cell_rise = generate_data_table("cell_rise", "rising_edge", pin, related_pin, corner.delay_template,
                                             corner)
        self.cell_fall = generate_data_table("cell_fall", "rising_edge", pin, related_pin, corner.delay_template,
                                             corner)
        self.rise_transition = generate_data_table("rise_transition", "rising_edge", pin, related_pin,
                                                   corner.delay_template, corner)
        self.fall_transition = generate_data_table("fall_transition", "rising_edge", pin, related_pin,
                                                   corner.delay_template, corner)
...
def require_key(obj, key):
    if key not in obj.attr.keys():
        sys.stderr.write(
            "Missing required key \"%s\" for %s object %s. Aborting.\n" % (key, obj.__class__.__name__, obj.name))
        exit(1)
...
def require_boolean(obj, key, default=None):
    if key in obj.attr:
        if type(obj.attr[key]) != type(False):
            sys.stderr.write(
                "Invalid entry \"%s\" for attribute \"%s\" of %s object %s. Must be true or false. Aborting.\n" % (
...
def require_values(obj, key, values, default=None):
    ...
    if obj.attr[key] not in values:
        sys.stderr.write(
            "Invalid entry \"%s\" for attribute \"%s\" of %s object %s. Allowed values are %s. Aborting.\n" % (
